...
pub struct NetworkMetadataEntry {
	...
	/// Empty = root network.
	pub network_path: Vec<RuntimeNodeId>,
	/// JSON of the runtime `PTZ` type.
	pub navigation_ptz: Option<serde_json::Value>,
	/// JSON of `DAffine2` (node-graph-space → viewport-space).
	pub navigation_transform: Option<serde_json::Value>,
	pub navigation_width: Option<f64>,
	/// JSON of `Previewing`. `None` means the runtime default `Previewing::No`.
...
pub trait NodeMetadataSource {
	...
	/// Runtime default `Previewing::No` maps to `None` so nodes don't gain an inert flag in storage.
